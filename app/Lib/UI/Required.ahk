...
class Required
{
    InputBox(prompt, title := "", throwOnFailure := true)
    {
        ...
        {
            if (throwOnFailure) {
                throw new Core.ValidationException(A_ThisFunc, "You must supply an input for '" prompt "' to continue.")
            } else {
    ...
    YesNoBox(prompt, title := "", throwOnFailure := true)
    {
        ...
        {
            if (throwOnFailure) {
                throw new Core.ValidationException(A_ThisFunc, "You must respond yes or no to '" prompt "' to continue.")
            } else {
